...
export type StyleObject = Partial<CSSStyleDeclaration> & { hover?: Partial<CSSStyleDeclaration> };

export function createStyles<T extends Record<string, StyleObject>>(styles: T): T {
    return styles;
}

type EventHandler = (e: Event) => void;
type ReactiveVal<T> = T | Signal<T>;
type PrimitiveAttr = ReactiveVal<string | number | boolean> | StyleObject | HTMLElement | EventHandler;
...
type tag_attributes<TTag extends keyof HTMLElementTagNameMap> = Partial<
    Record<TypAttributeKey<TTag>, any>
> & {
    style?: StyleObject | Signal<StyleObject>;
    hover?: HoverAttr;
    class?: any;
    className?: any;
};
...
function applyStyle(element: HTMLElement, styleObj: StyleObject) {
    const { hover, ...baseStyle } = styleObj;

    for (const [k, v] of Object.entries(baseStyle)) {
        if (v instanceof Signal) {
            v.subscribe(() => {
                (element.style as any)[k] = String(v.value ?? '');
            });
            (element.style as any)[k] = String(v.value ?? '');
        } else {
    ...
    if (hover) {
        ...
        for (const key in hover) {
            initialState[key as any] = (element.style as any)[key];
        }
...
function appendChildNode(element: HTMLElement, child: any) {
    if (child === null || child === undefined) return;

    if (child instanceof Signal) {
        const startComment = document.createComment('sig-start');
        const endComment = document.createComment('sig-end');
        element.appendChild(startComment);
        element.appendChild(endComment);

        const updateContent = () => {
            const val = child.value;
            while (startComment.nextSibling && startComment.nextSibling !== endComment) {
                element.removeChild(startComment.nextSibling);
            }
            ...
            if (Array.isArray(val)) {
                val.forEach(item => {
                    const node = typeof item === 'string' || typeof item === 'number'
                        ? document.createTextNode(String(item))
                        : item;
                    element.insertBefore(node, endComment);
                });
            } else if (val instanceof HTMLElement || val instanceof Text) {
                element.insertBefore(val, endComment);
            } else {
                element.insertBefore(document.createTextNode(String(val)), endComment);
            }
        };

        effect(updateContent);
    } else if (Array.isArray(child)) {
        child.forEach(c => appendChildNode(element, c));
    } else if (typeof child === 'string' || typeof child === 'number') {
        element.appendChild(document.createTextNode(String(child)));
    } else if (child instanceof HTMLElement || child instanceof Text || child instanceof DocumentFragment || child instanceof Comment) {
...
function createElement<TTag extends keyof HTMLElementTagNameMap>(
    ...
    for (const [rawKey, value] of Object.entries(parsedAttrs)) {
...
/**
 * Structural Helper: Conditional Rendering
 */
export function when(
    ...
    effect(() => {
        container.innerHTML = '';
        const isTrue = Boolean(conditionSignal.value);
        const content = isTrue ? thenBranch() : (elseBranch ? elseBranch() : null);
        if (content) {
            appendChildNode(container, content);
        }
    });
...
/**
 * Structural Helper: List Rendering
 */
export function forList<T>(
    listSignal: Signal<T[]>,
    ...
    effect(() => {
        ...
        items.forEach((item, index) => {
            ...
            appendChildNode(container, node);
        });
    });
...
/**
 * Form Helper: Two-way binding for inputs
 */
export function bind(signalInstance: Signal<any>) {
    return {
        value: signalInstance,
        oninput: (e: Event) => {
            const target = e.target as HTMLInputElement | HTMLTextAreaElement | HTMLSelectElement;
            if (target.type === 'checkbox') {
                signalInstance.value = (target as HTMLInputElement).checked;
            } else {
                signalInstance.value = target.value;
            }
        }
    };
}
